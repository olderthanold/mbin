...
# Scope: per-user shell setup + optional user cloning.
...
echo "=========================================="
echo "Running initusr (user setup)"
echo "User for user-level tasks: $CURRENT_USER"
echo "=========================================="

echo ""
echo "[1/2] mbin_path.sh - ensure ~/mbin, PATH update, and prompt cwd for user: $CURRENT_USER"
...
if [[ -n "$TARGET_USER" ]]; then
  echo "[2/2] clone_user.sh - create '$TARGET_USER' cloned from '$CURRENT_USER' (sudo + home + ssh keys)"
  ...
  echo "[2/2] clone_user.sh - skipped (no new username provided)"
fi
